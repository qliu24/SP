...
scale_save_file = os.path.join(Score_dir, 'scale_{}_{}_test.pickle'.format(category, set_type))
with open(scale_save_file,'rb') as fh:
    scale_record = pickle.load(fh)
    
score_save_file = os.path.join(Score_dir, 'score_{}_{}_{}_test.pickle'.format(category, set_type, VC['layer']))
with open(score_save_file,'rb') as fh:
    score_map = pickle.load(fh)
    
sp_num = score_map[0].shape[2]
# N = len(scale_record)
N=2000
...
# simple BG
inp_ls = [([score_map[nn][:,:,pp] - np.max(score_map[nn][:,:,[-2,-1]], axis=2) for nn in range(0,N,20)], \
           [spanno[nn][pp] for nn in range(0,N,20)], img_size[0:N:20], 1) for pp in range(sp_num-2)]
